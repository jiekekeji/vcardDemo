...
public class VCardComposer {
	...
	static {
		...
		// 地址类型
		addressTypeMap.put(Contacts.ContactMethods.TYPE_HOME, "HOME");
		addressTypeMap.put(Contacts.ContactMethods.TYPE_WORK, "WORK");
	}
	...
	public String createVCard(ContactStruct struct, int vcardversion)
		...
		if (!isNull(struct.company)) {
			mResult.append("ORG:").append(struct.company);
			// 部门
			if (!isNull(struct.departmentName)) {
				mResult.append(";").append(struct.departmentName)
						.append(mNewline);
			} else {
		...
		if (struct.notes != null) {
			for (String note : struct.notes) {
				if (!isNull(note)) {
					mResult.append("NOTE:").append(note).append(mNewline);
				}
			}
		}
		...
		// 网址
		if (struct.websiteList != null && struct.websiteList.size() > 0) {
			for (String website : struct.websiteList) {
				if (!isNull(website)) {
					mResult.append("URL:").append(website).append(mNewline);
				}
			}
		}
		...
		// 即时通讯
		if (!isNull(struct.qq)) {
			mResult.append("X-QQ:").append(struct.qq).append(mNewline);
		}

		if (!isNull(struct.msn)) {
			mResult.append("X-MSN:").append(struct.msn).append(mNewline);
		}
	...
	private void appendContactMethodStr(
		...
		for (ContactStruct.ContactMethod contactMethod : contactMList) {
			...
			case Contacts.KIND_ADR:
				String addressType = "HOME";
				if (!isNull(contactMethod.data)) {
					int methodType = new Integer(contactMethod.type).intValue();
					if (emailTypeMap.containsKey(methodType)) {
						addressType = emailTypeMap.get(methodType);
					} else if (!isNull(contactMethod.label)
							&& emailTypes.contains(contactMethod.label
									.toUpperCase())) {
						addressType = contactMethod.label.toUpperCase();
					}
					if (addressMap.containsKey(contactMethod.data)) {
						addressType = addressMap.get(contactMethod.data)
								+ joinMark + addressType;
					}
					addressMap.put(contactMethod.data, addressType);
				}
		...
		for (Map.Entry<String, String> email : addressMap.entrySet()) {
			if (version == VERSION_VCARD21_INT) {
				mResult.append("ADR;");
			} else {
